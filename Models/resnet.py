...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # 배치노말라이제이션 -> 배치 단위로 '정규화'를 수행
        # CIFAR 10 -> 개, 고양이, 사슴 / 트럭, 오토바이, 비행기
        ...
        #'잔차'는 1*1 Conv가 필요함 -> 레이어를 거친 F(x)와 크기가 동일
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        #AVGPool -> 최종 산출되는 특징맵의 크기를 고정시켜줌
        self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        #Linear의 앞부분 -> 특징*너비*높이
        #16*7*7
        self.fc = nn.Linear(512*1*1, num_classes) #CIFAR10
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        # x = [배치, 특징, 너비, 높이]
        x = x.flatten(1) #flatten(start_dim=0, end_dim=-1)
        x = self.fc(x)
